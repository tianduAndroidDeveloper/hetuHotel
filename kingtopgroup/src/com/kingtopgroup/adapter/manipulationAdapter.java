...
public View getView(int position, View convertView, ViewGroup parent) {
	...
	if (holder == null) {  
		...
		holder.item_id=(LinearLayout) convertView.findViewById(R.id.item_id);
	    ...
	    //convertView.setTag((Integer) getItem(position),ItemInfoList.get(position).get("pid"));
	    holder.order_item_name.setId(Integer.parseInt((String) ItemInfoList.get(position).get("pid")));
	    //holder.item_id.setId(Integer.parseInt((String) ItemInfoList.get(position).get("pid")));
		holder.order_item_name.setText((CharSequence) ItemInfoList.get(position).get("name"));
		holder.order_item_time.setText((CharSequence) ItemInfoList.get(position).get("time"));
		holder.beginnum.setText((CharSequence) ItemInfoList.get(position).get("beginnum")+"ÈËÆð¶©");
		holder.order_item_price.setText((CharSequence) ItemInfoList.get(position).get("marketprice"));
		finalBitMap.display(holder.order_item_image1, (String) ItemInfoList.get(position).get("order_item_image1"));  
