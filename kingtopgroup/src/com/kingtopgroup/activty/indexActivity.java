	...
	private void setMenu_cooper(){
		if (cooper_window != null && cooper_window.isShowing()) {
			...
			SimpleAdapter listAdapter = new SimpleAdapter(
					indexActivity.this, listLeft, R.layout.person_menu_item,
					new String[] { "item" },
	...
	private void setMenu_more(){
		if (more_window != null && more_window.isShowing()) {
			...
			more_view = getLayoutInflater().inflate(
					R.layout.person_menu_listview, null);
			more_listview = (ListView) more_view
					.findViewById(R.id.menulist);
			SimpleAdapter listAdapter = new SimpleAdapter(
					indexActivity.this, setmoreMenu(), R.layout.person_menu_item,
					new String[] { "item" },
					new int[] { R.id.menuitem });
			...
			// 设置popupwindow的位置（相对tvLeft的位置）
			//int topBarHeight = rlTopBar.getBottom();
			more_window.showAsDropDown(more, 0,
					(180 - more.getHeight()) / 2);
